...
public class EvaluationFunctionA implements EvaluationFunction {
    ...
    public static final EvaluationFunctionA EQUAL_WEIGHTS	       = new EvaluationFunctionA("Equal Weights", 1, 1,
            1, 1);
    public static final EvaluationFunctionA GREEDY_WEIGHTS_STONECOUNT  = new EvaluationFunctionA("Stonecount", 0.0,
            10.0, 0.0, 0.0);
    public static final EvaluationFunctionA GREEDY_WEIGHTS_CENTER      = new EvaluationFunctionA("Center Location", 1.0,
            0.0, 0.0, 0.0);
    public static final EvaluationFunctionA GREEDY_WEIGHTS_DIAGONAL    = new EvaluationFunctionA("Diagonal Location",
            0.0, 0.0, 1.0, 0.0);
    public static final EvaluationFunctionA GREEDY_WEIGHTS_LINEOFTHREE = new EvaluationFunctionA("Line Of Three", 0.0,
            0.0, 0.0, 1.0);
    ...
    public EvaluationFunctionA(String name, double centerWeight, double stoneCountWeight, double diagonalWeight,
            double lineOfThreeWeight) {
	...
	int[][] edgeCoor = new int[][]{{1, 1}, {1, 2}, {1, 3}, {1, 4}, {2, 5}, {3, 6}, {4, 7}, {5, 7}, {6, 7}, {7, 7},
	        {7, 6}, {7, 5}, {7, 4}, {6, 3}, {5, 2}, {4, 1}, {3, 1}, {2, 1}};
	...
	for (int i = 1; i < grid.length - 1; i++) {
		...
		if (((prevValue == Board.WHITE_VALUE) || (prevValue == Board.GIPF_WHITE_VALUE))
		        && ((grid[i][j] == Board.WHITE_VALUE) || (grid[i][j] == Board.GIPF_WHITE_VALUE))) {
		    counter++;
		} else if (((prevValue == Board.BLACK_VALUE) || (prevValue == Board.GIPF_BLACK_VALUE))
		        && ((grid[i][j] == Board.BLACK_VALUE) || (grid[i][j] == Board.GIPF_BLACK_VALUE))) {
		    counter++;
		} else {
		    if (counter >= 3) {
	...
	for (int k = 0; k < 4; k++) {
	    for (int l = 0; l < 5 + k; l++) {
		if (((prevValue == Board.WHITE_VALUE) || (prevValue == Board.GIPF_WHITE_VALUE))
		        && ((grid[l + 1][4 - k + l] == Board.WHITE_VALUE)
		                || (grid[l + 1][4 - k + l] == Board.GIPF_WHITE_VALUE))) {
		    counter++;
		} else if (((prevValue == Board.BLACK_VALUE) || (prevValue == Board.GIPF_BLACK_VALUE))
		        && ((grid[l + 1][4 - k + l] == Board.BLACK_VALUE)
		                || (grid[l + 1][4 - k + l] == Board.GIPF_BLACK_VALUE))) {
		    counter++;
		} else {
		    if (counter >= 3) {
			if (prevValue == Board.WHITE_VALUE)
	...
	for (int k = 0; k < 3; k++) {
	    for (int l = 0; l < 7 - k; l++) {
		if (((prevValue == Board.WHITE_VALUE) || (prevValue == Board.GIPF_WHITE_VALUE))
		        && ((grid[2 + k + l][1 + l] == Board.WHITE_VALUE)
		                || (grid[2 + k + l][1 + l] == Board.GIPF_WHITE_VALUE))) {
		    counter++;
		} else if (((prevValue == Board.BLACK_VALUE) || (prevValue == Board.GIPF_BLACK_VALUE))
		        && ((grid[2 + k + l][1 + l] == Board.BLACK_VALUE)
		                || (grid[2 + k + l][1 + l] == Board.GIPF_BLACK_VALUE))) {
		    counter++;
		} else {
		    if (counter >= 3) {
			if (prevValue == Board.WHITE_VALUE)
