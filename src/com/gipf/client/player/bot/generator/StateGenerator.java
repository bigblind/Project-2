...
public class StateGenerator {
	...
	public ArrayList<GameState> generateStates(GameState gameState, Player player, GameLogic logic) { //TODO use to do the moves
		...
		for (int i = 5; i < 8; i++) {
			...
			to1 = new Point(i - 1, 7);
			to2 = new Point(i, 7);
			if (board.isValidMove(from, to1)) this.addState(gameState, player, from, to1, states);
			if (board.isValidMove(from, to2)) this.addState(gameState, player, from, to2, states);
		}
		// correct, right
		for (int j = 7; j >= 5; j--) {
			from = new Point(8, j);
		...
		from = new Point(8, 4);
		to1 = new Point(7, 4);
		if (board.isValidMove(from, to1)) addState(gameState, player, from, to1, states);
	...
	private void addState(GameState gameState, Player player, Point from, Point to, ArrayList<GameState> states) {
		...
		states.add(new GameState(tmp, from, to));
	}
}
