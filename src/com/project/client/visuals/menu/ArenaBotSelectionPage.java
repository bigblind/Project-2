...
public class ArenaBotSelectionPage extends MenuPage {

	private static final long serialVersionUID = 634389002387668782L;

	private final BotSelectionPanel bot1Panel, bot2Panel;
	private JLabel title;
	private JButton back, start;
	
	private String mode;
	
	public ArenaBotSelectionPage(final Controller controller) {
		super(controller);
		...
		this.bot1Panel = new BotSelectionPanel(controller, "Bot One");
		this.bot2Panel = new BotSelectionPanel(controller, "Bot Two");
		
		Box box = Box.createHorizontalBox();
		box.add(Box.createHorizontalStrut(80));
		box.add(this.bot1Panel);
		box.add(Box.createHorizontalStrut(20));
		box.add(Box.createHorizontalGlue());
		box.add(Box.createHorizontalStrut(20));
		box.add(this.bot2Panel);
		box.add(Box.createHorizontalStrut(80));
		center.add(box);
		...
		this.start.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				controller.createArenaGame(new LocalServer(null, null, mode), bot1Panel.getBot(), bot2Panel.getBot());
