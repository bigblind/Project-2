...
//	}
	...
	public Node<E> maxValue(Node<E> node, Node<E> alpha, Node<E> beta, int depth){
		if(depth == 0 || tree.isExternal(node)) return node;
...
//			alpha = Math.max(alpha.getEvalValue(), minValue(child, currAlpha, beta, depth-1).getEvalValue());
			alpha = minValue(child, currAlpha, beta, depth-1);
			
			if(currAlpha.getEvalValue() >= alpha.getEvalValue())
				alpha = currAlpha;
			
			if(alpha.getEvalValue() >= beta.getEvalValue()) return beta;	
		}
		return alpha;
	}
	...
	public Node<E> minValue(Node<E> node, Node<E> alpha, Node<E> beta, int depth){
		...
		while(children.hasNext()){
			child = (Node<E>) children.next();
//			beta = Math.min(beta.getEvalValue(), maxValue(child, alpha, currBeta, depth-1).getEvalValue());
			
			beta = maxValue(child, alpha, currBeta, depth-1);
			
			if(currBeta.getEvalValue() <= beta.getEvalValue())
				beta = currBeta;
						
			if(beta.getEvalValue() <= alpha.getEvalValue()) return alpha;
		}
		return beta;
	}
