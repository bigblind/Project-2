...
public class MoveGenerator {
	...
	public MoveGenerator(Game game) {
		this.game = game;
		this.tree = new Tree<Integer>(new Node<Integer>(game.getBoard(), null));
	}
	...
	private ArrayList<Node<Integer>> generateMove(int ply, Node<Integer> parent, Player player) {
		Player nextPlayer;
		if (player == game.getPlayerOne()) nextPlayer = game.getPlayerTwo();
		...
		if (ply > 0) {
			...
			for (int i = 5; i < 8; i++) {
				...
				to1 = new Point(i - 1, 7);
				to2 = new Point(i, 7);
				if (board.isValidMove(from, to1)) result.add(addMove(from, to1, parent, player, nextPlayer, ply - 1));

				if (board.isValidMove(from, to2)) result.add(addMove(from, to2, parent, player, nextPlayer, ply - 1));
			}
			// correct, right
			for (int j = 7; j >= 5; j--) {
				from = new Point(8, j);
