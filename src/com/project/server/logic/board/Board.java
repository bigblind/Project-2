			...
			for (int j = 1; j < grid[i].length; j++) {
				if (prevValue == grid[i][j] && prevValue > 0) {
					...
					if (counter >= 3) {
						...
						lineStartY = j - counter;
						int[] extensionStones = getExtensionStones(new Point(lineStartX, lineStartY), new Point(lineEndX, lineEndY));
						lines.add(new Row(new Point(lineStartX, lineStartY), new Point(lineEndX, lineEndY), logic.checkPlayer(prevValue), counter + 1, extensionStones[0], extensionStones[1]));
						counter = 0;
					}
					counter = 0;
					prevValue = grid[i][j];
